import React from 'react';
import ChwEditFormProps from './ChwEditFormProps';
import { FormStatus, readEventValueAsNum } from '../../../../api/chws';
import { Box, Button, Form, Heading } from 'grommet';
import Spinner from '../../../../components/Spinner/Spinner';
import ChwFormField from './ChwFormField';

const WeightHeightChecksDutiesForm = (props: ChwEditFormProps) => {
  const { report, onSubmit, updateFn, formEvent } = props;

  if (formEvent.status === FormStatus.Submit) {
    return (
      <Box direction={'column'} fill>
        <Spinner size={228} />
      </Box>
    );
  }

  return (
    <Box
      direction={'column'}
      fill
      margin={{ left: 'medium' }}
      overflow={'auto'}
    >
      <Heading level={2}>Duties Performed: Weight & Height Checks</Heading>
      <Form onSubmit={onSubmit}>
        <Box
          direction={'row'}
          align={'start'}
          gap={'small'}
          margin={{ right: 'large', top: 'small' }}
        >
          <ChwFormField
            label={'1 - 4: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck.oneToFour.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck.oneToFour
                .female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      oneToFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .oneToFour,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      oneToFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .oneToFour,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'oneToFour.male'}
            femaleFieldName={'oneToFour.female'}
          />
          <ChwFormField
            label={'5 - 9: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck.fiveToNine.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck.fiveToNine
                .female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fiveToNine: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fiveToNine,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fiveToNine: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fiveToNine,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'fiveToNine.male'}
            femaleFieldName={'fiveToNine.female'}
          />
          <ChwFormField
            label={'10 - 14: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck.tenToFourteen
                .male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck.tenToFourteen
                .female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      tenToFourteen: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .tenToFourteen,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      tenToFourteen: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .tenToFourteen,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'tenToFourteen.male'}
            femaleFieldName={'tenToFourteen.female'}
          />
          <ChwFormField
            label={'15 - 19: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .fifteenToNineteen.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .fifteenToNineteen.female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fifteenToNineteen: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fifteenToNineteen,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fifteenToNineteen: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fifteenToNineteen,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'fifteenToNineteen.male'}
            femaleFieldName={'fifteenToNineteen.female'}
          />
        </Box>
        <Box
          direction={'row'}
          align={'start'}
          gap={'small'}
          margin={{ right: 'large', top: 'small' }}
        >
          <ChwFormField
            label={'20 - 24: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .twentyToTwentyFour.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .twentyToTwentyFour.female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      twentyToTwentyFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .twentyToTwentyFour,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      twentyToTwentyFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .twentyToTwentyFour,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'twentyToTwentyFour.male'}
            femaleFieldName={'twentyToTwentyFour.female'}
          />
          <ChwFormField
            label={'25 - 29: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .twentyFiveToTwentyNine.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .twentyFiveToTwentyNine.female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      twentyFiveToTwentyNine: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .twentyFiveToTwentyNine,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      twentyFiveToTwentyNine: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .twentyFiveToTwentyNine,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'twentyFiveToTwentyNine.male'}
            femaleFieldName={'twentyFiveToTwentyNine.female'}
          />
          <ChwFormField
            label={'30 - 34: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .thirtyToThirtyFour.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .thirtyToThirtyFour.female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      thirtyToThirtyFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .thirtyToThirtyFour,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      thirtyToThirtyFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .thirtyToThirtyFour,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'thirtyToThirtyFour.male'}
            femaleFieldName={'thirtyToThirtyFour.female'}
          />
          <ChwFormField
            label={'35 - 39: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .thirtyFiveToThirtyNine.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .thirtyFiveToThirtyNine.female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      thirtyFiveToThirtyNine: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .thirtyFiveToThirtyNine,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      thirtyFiveToThirtyNine: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .thirtyFiveToThirtyNine,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'thirtyFiveToThirtyNine.male'}
            femaleFieldName={'thirtyFiveToThirtyNine.female'}
          />
        </Box>
        <Box
          direction={'row'}
          align={'start'}
          gap={'small'}
          margin={{ right: 'large', top: 'small' }}
        >
          <ChwFormField
            label={'40 - 44: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .fortyToFortyFour.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .fortyToFortyFour.female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fortyToFortyFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fortyToFortyFour,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fortyToFortyFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fortyToFortyFour,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'fortyToFortyFour.male'}
            femaleFieldName={'fortyToFortyFour.female'}
          />
          <ChwFormField
            label={'45 - 49: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .fortyFiveToFortyNine.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .fortyFiveToFortyNine.female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fortyFiveToFortyNine: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fortyFiveToFortyNine,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fortyFiveToFortyNine: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fortyFiveToFortyNine,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'fortyFiveToFortyNine.male'}
            femaleFieldName={'fortyFiveToFortyNine.female'}
          />
          <ChwFormField
            label={'50 - 54: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .fiftyToFiftyFour.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .fiftyToFiftyFour.female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fiftyToFiftyFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fiftyToFiftyFour,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fiftyToFiftyFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fiftyToFiftyFour,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'fiftyToFiftyFour.male'}
            femaleFieldName={'fiftyToFiftyFour.female'}
          />
          <ChwFormField
            label={'55 - 59: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .fiftyFiveToFiftyNine.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .fiftyFiveToFiftyNine.female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fiftyFiveToFiftyNine: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fiftyFiveToFiftyNine,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      fiftyFiveToFiftyNine: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .fiftyFiveToFiftyNine,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'fiftyFiveToFiftyNine.male'}
            femaleFieldName={'fiftyFiveToFiftyNine.female'}
          />
        </Box>
        <Box
          direction={'row'}
          align={'start'}
          gap={'small'}
          margin={{ right: 'large', top: 'small' }}
        >
          <ChwFormField
            label={'60 - 64: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .sixtyToSixtyFour.male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck
                .sixtyToSixtyFour.female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      sixtyToSixtyFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .sixtyToSixtyFour,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      sixtyToSixtyFour: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .sixtyToSixtyFour,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'sixtyToSixtyFour.male'}
            femaleFieldName={'sixtyToSixtyFour.female'}
          />
          <ChwFormField
            label={'65+: '}
            maleValue={
              report.report.dutiesPerformed.weightAndHeightCheck.sixtyFiveOrMore
                .male
            }
            femaleValue={
              report.report.dutiesPerformed.weightAndHeightCheck.sixtyFiveOrMore
                .female
            }
            maleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      sixtyFiveOrMore: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .sixtyFiveOrMore,
                        male: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            femaleOnChange={(e) => {
              updateFn({
                ...report,
                report: {
                  ...report.report,
                  dutiesPerformed: {
                    ...report.report.dutiesPerformed,
                    weightAndHeightCheck: {
                      ...report.report.dutiesPerformed.weightAndHeightCheck,
                      sixtyFiveOrMore: {
                        ...report.report.dutiesPerformed.weightAndHeightCheck
                          .sixtyFiveOrMore,
                        female: readEventValueAsNum(e),
                      },
                    },
                  },
                },
              });
            }}
            maleFieldName={'sixtyFiveOrMore.male'}
            femaleFieldName={'sixtyFiveOrMore.female'}
          />
        </Box>
        <Box flex align={'start'} margin={{ left: 'xxsmall', top: 'medium' }}>
          <Button type={'submit'} label={'Save'} primary />
        </Box>
      </Form>
    </Box>
  );
};

export default WeightHeightChecksDutiesForm;
